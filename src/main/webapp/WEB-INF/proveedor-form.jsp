...
<div class="container">
    ...
    <div class="card">
        <div class="card-pad">
            ...
            <h2>
                <c:choose>
                    <c:when test="${isUpdate}">✏️ Editar Proveedor</c:when>
                    <c:otherwise>👥 Nuevo Proveedor</c:otherwise>
                </c:choose>
            </h2>

            <form method="post" action="${urlProveedores}">
                <input type="hidden" name="action" value="${formAction}"/>

                <c:if test="${isUpdate}">
                    <input type="hidden" name="idProveedor" value="${proveedor.idProveedor}"/>
                    <p class="muted">ID: <span class="id-pill"><c:out value="${proveedor.idProveedor}"/></span></p>
                </c:if>

                <div class="field">
                    <label>Nombre:</label>
                    <input type="text" name="nombre" value="${proveedor.nombre}" maxlength="120" required/>
                </div>

                <div class="field">
                    <label>RUC:</label>
                    <input type="text" name="ruc" value="${proveedor.ruc}" maxlength="13" required/>
                </div>

                <div class="actions">
                    <button class="btn btn-primary" type="submit">Guardar</button>
                    <a class="btn" href="${urlProveedores}">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
